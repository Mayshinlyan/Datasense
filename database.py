...
async def initialize_vector_store(self):
    ...
    try:
        ...
        logger.info("Created AlloyDBEngine instance.")

        # Initialize the vector store
        self.vector_store = await AlloyDBVectorStore.create_sync(
            self.engine,
        ...
        logger.error(f"Error initializing AlloyDBEngine or vector store: {e}")
        raise
...
def similarity_search(self, query: str) -> list[Document]:
    ...
    """
    initialize_vector_store(self)
    logger.info(f"Querying the vector store with query: {query}")
    docs = self.vector_store.similarity_search(query, k=5)
    logger.info(f"The docs are {docs}")
...
def _create_dataframe_from_results(
    ...
    async def main():
        vec = VectorStore()
        results = vec.similarity_search("hi")
